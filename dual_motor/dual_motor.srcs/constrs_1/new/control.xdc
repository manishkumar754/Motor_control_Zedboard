set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {MOTOR[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SWITCH[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports CLK]
 
set_property PACKAGE_PIN Y11 [get_ports {MOTOR[0]}]
set_property PACKAGE_PIN AA11 [get_ports {MOTOR[1]}]
set_property PACKAGE_PIN Y10 [get_ports {MOTOR[2]}]
set_property PACKAGE_PIN AB11 [get_ports {MOTOR[3]}]
set_property PACKAGE_PIN AB10 [get_ports {MOTOR[4]}]
set_property PACKAGE_PIN AB9 [get_ports {MOTOR[5]}]
 
set_property PACKAGE_PIN Y9 [get_ports CLK]
 
set_property PACKAGE_PIN F22 [get_ports {SWITCH[0]}]
set_property PACKAGE_PIN G22 [get_ports {SWITCH[1]}]
set_property PACKAGE_PIN H22 [get_ports {SWITCH[2]}]
set_property PACKAGE_PIN F21 [get_ports {SWITCH[3]}]
set_property PACKAGE_PIN H19 [get_ports {SWITCH[4]}]
set_property PACKAGE_PIN H18 [get_ports {SWITCH[5]}]